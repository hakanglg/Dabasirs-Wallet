...
class AppbarMonthsTab extends StatelessWidget implements PreferredSizeWidget {
  const AppbarMonthsTab({
    super.key,
  ...
  List<Tab> _myTabs(BuildContext context) {
    return List.generate(12, (index) {
      var date = DateTime(DateTime(2022).year, DateTime(2022).month + index,
          DateTime(2022).day);
      String yearDisplay = DateFormat.y().format(date);
      String monthDisplay = DateFormat.MMMM(
              context.locale.languageCode.toString() +
                  context.locale.countryCode.toString())
          .format(date);
      return _TabBarSized(context, yearDisplay, monthDisplay);
    });
  }

  Tab _TabBarSized(
      BuildContext context, String yearDisplay, String monthDisplay) {
    return Tab(
        child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(child: Text(monthDisplay)),
        ],
      ),
    ));
  }
